...
if [ -z "$JEYZER_GRAPH_REFRESH_PERIOD" ]; then
  JEYZER_GRAPH_REFRESH_PERIOD=4s
...
JEYZER_ANALYZER_REPLAY_ENABLED=true
...
PARAMS="-Djeyzer.analysis.config="$JEYZER_TARGET_PROFILES_DIR"/"$JEYZER_TARGET_PROFILE"/"$JEYZER_TARGET_PROFILE"_analysis.xml -Dlogback.configurationFile="$JEYZER_ANALYZER_CONFIG_DIR"/log/replay-logback.xml"
export PARAMS
