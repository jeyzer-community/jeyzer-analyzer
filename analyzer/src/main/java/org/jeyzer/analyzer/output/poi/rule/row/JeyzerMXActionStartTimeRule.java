...
public class JeyzerMXActionStartTimeRule extends AbstractDisplayRule implements RowHeader{
	
	public static final String RULE_NAME = "jzr_mx_action_start_time";
	
	private static final String DISPLAY_NAME = "Start time (MX)";
	
	public static final String CELL_LABEL_COMMENT = "Action start date obtained through the Jeyzer MX interface";
	
	private boolean hasLink;
	
	public JeyzerMXActionStartTimeRule(ConfigDisplay headerCfg, SheetDisplayContext context){
	...
	public boolean apply(ThreadAction action, Cell cell) {
		...
		if (action.getThreadStackJeyzerMXInfo().getStartTime() != -1){
			cell.setCellStyle(this.context.getCellStyles().getThemeStyles().getThemeStyle(STYLE_THEME_HEADER_DATE_ROW));		
			cell.setCellValue(new Date(action.getThreadStackJeyzerMXInfo().getStartTime()));
		}else{
