...
public class JeyzerMXActionPrincipalRule  extends AbstractDisplayRule implements RowHeader{

	public static final String RULE_NAME = "jzr_mx_action_function_principal";
	
	private static final String DISPLAY_NAME = "Action principal";
	
	public static final String CELL_LABEL_COMMENT = "Action principal obtained through the Jeyzer MX interface";
	
	private boolean hasLink;
	
	public JeyzerMXActionPrincipalRule(ConfigDisplay headerCfg, SheetDisplayContext context){
	...
	public boolean apply(ThreadAction action, Cell cell) {
		...
		if (action.getThreadStackJeyzerMXInfo() != null 
				&& action.getThreadStackJeyzerMXInfo().getFunctionPrincipal() != null
				&& !action.getThreadStackJeyzerMXInfo().getFunctionPrincipal().isEmpty()){
			value = action.getThreadStackJeyzerMXInfo().getFunctionPrincipal();			
		}
		else{
			value = NOT_AVAILABLE;
