...
public class OTBIOfInterestRule extends AbstractCellDisplayRule implements DisplayRule {

	private static final Logger logger = LoggerFactory.getLogger(OTBIOfInterestRule.class);
	
	public static final String RULE_NAME = "otbi_of_interest";
	public static final String THRESHOLD_NAME = "threshold";
	
	private int threshold = 10;
	
	public OTBIOfInterestRule (ConfigDisplay displayCfg, SequenceSheetDisplayContext context){
	...
	public boolean apply(Action action, List<Cell> cells) {
		...
		for (int j=0; j< action.size(); j++){
			...
			if (stack.isOTBI() && stack.getStackHandler().getCodeLines().size() >= threshold){
				setColorForeground(cell);
				if (cell.getStringCellValue().isEmpty())
					setValue(cell, ThreadStack.OPER_TO_BE_IDENTIFIED);
			}
		}
	...
	public int displayLegend(int line, int pos) {
		return displayLegend("Operation to be identified of interest : stack size >= " + threshold, ThreadStack.OPER_TO_BE_IDENTIFIED, true, line, pos);
	}
