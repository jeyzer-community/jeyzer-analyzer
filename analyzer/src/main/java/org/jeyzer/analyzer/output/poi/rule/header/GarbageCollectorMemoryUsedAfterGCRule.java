...
public class GarbageCollectorMemoryUsedAfterGCRule  extends AbstractGarbageCollectionMemoryRule implements Header {

	public static final String RULE_NAME = "garbage_collection_memory_used_after_gc";

	public static final String CELL_LABEL_COMMENT = "Memory still in use after garbage garbage collection in Mb for";	
	...
	public GarbageCollectorMemoryUsedAfterGCRule(ConfigSheetHeader headerCfg, SheetDisplayContext context) {
	...
	public boolean apply(List<ThreadDump> dumps, List<Cell> cells, HeaderFunction function) {
		...
		for (int j=0; j< dumps.size(); j++){
			...
			memoryUsedValue = buildGCExecutionValue(gcInfo, 
					gcInfo != null ? gcInfo.getUsedMemoryAfter():0,
	...
	protected long getMemoryPoolValue(GarbageCollectorMemoryPool memPool){
		return memPool.getUsedMemoryAfter();
	}
