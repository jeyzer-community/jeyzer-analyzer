...
public class JeyzerMXUserRule  extends AbstractDisplayRule implements RowHeader{

	public static final String RULE_NAME = "jzr_mx_action_user";
	
	private static final String DISPLAY_NAME = "Action user";
	
	public static final String CELL_LABEL_COMMENT = "Action user obtained through the Jeyzer MX interface";
	
	private boolean hasLink;
	
	public JeyzerMXUserRule(ConfigDisplay headerCfg, SheetDisplayContext context){
	...
	public boolean apply(ThreadAction action, Cell cell) {
		...
		if (action.getThreadStackJeyzerMXInfo() != null 
				&& action.getThreadStackJeyzerMXInfo().getUser() != null
				&& !action.getThreadStackJeyzerMXInfo().getUser().isEmpty()){
			value = action.getThreadStackJeyzerMXInfo().getUser();			
		}
		else {
			value = NOT_AVAILABLE;
