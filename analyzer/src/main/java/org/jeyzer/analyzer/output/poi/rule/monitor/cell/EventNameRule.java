...
public class EventNameRule extends AbstractCellDisplayRule implements MonitorDisplayRule {

	public static final String RULE_NAME = "event_name";
	
	public EventNameRule(ConfigDisplay displayCfg, SequenceSheetDisplayContext context) {
	...
	public boolean apply(MonitorEvent event, List<Cell> cells) {
		...
		for (int j=0; j< cells.size(); j++){
			...
			if (j % 10 == 0){
				String value = event instanceof MonitorApplicativeEvent ? "App / " + event.getMessage() : event.getName();
				setValue(cell, value);
