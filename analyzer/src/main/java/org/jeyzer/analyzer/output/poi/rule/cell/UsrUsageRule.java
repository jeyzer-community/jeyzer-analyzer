...
public class UsrUsageRule extends AbstractCPUDisplayRule implements DisplayRule {
	
	public static final String RULE_NAME = "usr_usage";
	
	public UsrUsageRule(ConfigDisplay displayCfg, SequenceSheetDisplayContext context){
	...
	public boolean apply(Action action, List<Cell> cells) {
		...
		for (int j=0; j< action.size(); j++){
			...
			displaySingleValue(cell, LABEL_USR_USAGE, (int)Math.round(stackCPUInfo.getUserUsage()), actionStatsContext, stack.getInstanceCount());
		}
