...
public class UsedPhysicalSystemMemoryRule extends AbstractNumericDisplayHeaderRule implements Header{
	
	public static final String RULE_NAME = "system_physical_used_memory";
	
	public static final String CELL_LABEL_COMMENT = "System physical used memory in Mb";
	...
	public UsedPhysicalSystemMemoryRule(ConfigSheetHeader headerCfg, SheetDisplayContext context){
	...
	public boolean apply(List<ThreadDump> dumps, List<Cell> cells, HeaderFunction function) {
		...
		for (int j=0; j< dumps.size(); j++){
			...
			usedMemory = dumps.get(j).getSystemPhysicalUsedMemory();
			if (Double.doubleToRawLongBits(usedMemory) != DOUBLE_TO_LONG_NA){
				// display in Mb
				usedMemory = FormulaHelper.convertToMb(usedMemory);
				value = usedMemory;
			}
			...
			setValueBasedColorForeground(
					...
					usedMemory,
					prevUsedMemory,
					Double.doubleToRawLongBits(usedMemory) == DOUBLE_TO_LONG_NA
					);
