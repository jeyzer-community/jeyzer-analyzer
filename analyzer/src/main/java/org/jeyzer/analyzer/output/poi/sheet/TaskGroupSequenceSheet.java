...
import org.jeyzer.analyzer.output.poi.rule.group.cell.GroupDisplayRuleBuilder;
import org.jeyzer.analyzer.output.poi.rule.group.row.GroupRowHeader;
import org.jeyzer.analyzer.output.poi.rule.group.row.GroupRowHeaderBuilder;
...
public class TaskGroupSequenceSheet extends JeyzerSheet {
	...
	private int stackGroupCount = 0;
	
	public TaskGroupSequenceSheet(ConfigSequenceSheet sheetCfg, JzrSession session, DisplayContext displayContext){
	...
	public void display() throws JzrReportException {
		...
		GroupRowHeaderBuilder rowHeaderBuilder = GroupRowHeaderBuilder.newInstance();
		List<GroupRowHeader> rowHeaders = rowHeaderBuilder.buildRowHeaders(sheetCfg.getRowHeadersConfig(), new SheetDisplayContext(displayContext, sheet));
		
		GroupDisplayRuleBuilder displayBuilder = GroupDisplayRuleBuilder.newInstance();
		int tdPeriod = session.getThreadDumpPeriod();
		List<DisplayRule> rules = displayBuilder.buildRules(sheetCfg.getDisplayConfigs(), new SequenceSheetDisplayContext(displayContext, sheet, tdPeriod), session);
		...
		updateTimeHeaderCells(sheet, rules, columnOffset);
		...
		this.itemsCount = rules.isEmpty() ? headers.size() : this.stackGroupCount;
		close(this.sheetCfg);
	}

	private int getHeaderRowOffset(List<GroupRowHeader> rowHeaders, List<HeaderFunction> modelFunctions) {
	...
	private void addSeries(Sheet sheet, List<Header> headers, ConfigChart chartConfig, XSSFLineChartData lineChartData, int rowHeaderSize) {
		...
		int x2 = rowHeaderSize + this.missingTds.size() - 1;
	...
	private int getRowFreezeHeaderSize(List<GroupRowHeader> rowHeaders) {
	...
	private void reportTaskSequenceHeader(Sheet sheet, List<GroupRowHeader> rowHeaders, List<HeaderFunction> functions, boolean headersOnly, int headerLabelsColumnSize){
		Row row0;
		Row row1;
		...
		int columnPos = 0;
		Cell cell;
		for (GroupRowHeader rowHeader : rowHeaders){
	...
	private void fillData(Sheet sheet, List<DisplayRule> rules, List<GroupRowHeader> rowHeaders, int rowOffset, int columnOffset){
		int offset = 0;
		Row row;
		Cell cell;
		List<Cell> cells;
		...
		for (ThreadDump dump : session.getDumps()){
			...
			Set<ThreadStackGroupAction> actions = session.getThreadStackGroupActionHistory().get(timestamp);
	...
	private void updateTimeHeaderCells(Sheet sheet, List<DisplayRule> rules, int columnOffset){
		int offset = 0;
		Row row;
		Cell cell;
	...
	private void prepareCellLinks(Row row, int colIndex, ThreadStackGroupAction action, Date timestamp, boolean firstAction) {
    ...
    private int fillLegend(Sheet sheet, List<DisplayRule> rules, List<GroupRowHeader> rowHeaders, int headerOffset, int columnOffset){
    	Row row;
		...
		if (hasRowHeaderLegend){
	    	...
	    	for (GroupRowHeader rule : rowHeaders){
	...
	private boolean hasRowHeaderLegend(List<GroupRowHeader> rowHeaders) {
    	for (GroupRowHeader rule : rowHeaders){
			if (rule.hasLegend()){
				return true;
			}
