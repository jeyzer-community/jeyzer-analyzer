...
public class PrincipalHistogramSheet extends JeyzerSheet{

	private ConfigPrincipalHistogramSheet sheetCfg;
	
	public PrincipalHistogramSheet(ConfigPrincipalHistogramSheet config, JzrSession session, DisplayContext displayContext) {
	...
	public void display() throws JzrException {
    	...
    	Map<String, ExcelGraphPicture> graphPictures = new LinkedHashMap<String, ExcelGraphPicture>();
    	Map<String, ExcelGraphPicture> contentionGraphPictures = new LinkedHashMap<String, ExcelGraphPicture>();
    	
    	linePos = displayTags(sheet, graphPictures, contentionGraphPictures, linePos);
    	
    	createFilters(sheet, linePos, rowPos);

    	linePos = displayGraphPictures(sheet, graphPictures, contentionGraphPictures, linePos, rowPos);
	...
	private int displayGraphPictures(Sheet sheet, Map<String, ExcelGraphPicture> graphPictures, Map<String, ExcelGraphPicture> contentionGraphPictures, int linePos, int rowPos) throws JzrException {
		...
		if (this.sheetCfg.isFunctionGraphDisplayed()){
			lines = new HashMap<String, Integer>(graphPictures.size());
			ExcelGraphPicture candidate = null;
			
			for (Map.Entry<String,ExcelGraphPicture> entry : graphPictures.entrySet()){
	...
	private int displayTags(Sheet sheet, Map<String, ExcelGraphPicture> graphPictures, Map<String, ExcelGraphPicture> contentionGraphPictures, int linePos) throws JzrException {
    	Multimap<String, Tag> tagsPerFunctionPrincipal = session.getFunctionSetPerFunctionPrincipal();
    	tagsPerFunctionPrincipal.putAll(session.getOperationSetPerFunctionPrincipal());
    	tagsPerFunctionPrincipal.putAll(session.getContentionTypeSetPerFunctionPrincipal());
    	this.itemsCount = tagsPerFunctionPrincipal.keySet().size();
    	...
    	int groupCount = 0;
    	for (String principal : Multisets.copyHighestCountFirst(tagsPerFunctionPrincipal.keys()).elementSet()) {
    		Collection<Tag> tags = tagsPerFunctionPrincipal.get(principal);
    		Collection<ThreadAction> actions = actionsPerFunctionPrincipal.get(principal);
    		
    		// build graph
   			buildGraphPictures(principal, actions, graphPictures, contentionGraphPictures);
    		...
    		for (Tag tag : Multisets.copyHighestCountFirst(tagMultiSet).elementSet()){
    			linePos++;
    			displayTag(principal, tag, tagMultiSet.count(tag), actionStackCount, globalActionsStackSize, sheet, groupCount, linePos, graphPictures, contentionGraphPictures, first);
    			first = false;
    		}
			groupCount++;
    	}
	...
	private void buildGraphPictures(String principal, Collection<ThreadAction> actions, Map<String, ExcelGraphPicture> graphPictures, Map<String, ExcelGraphPicture> contentionGraphPictures) throws JzrException {
		if(!this.sheetCfg.isFunctionGraphDisplayed() && !this.sheetCfg.isContentionGraphDisplayed())
			return;
		...
		buildFunctionGraphPicture(section, principal, graphPictures);
		buildContentionGraphPicture(section, principal, contentionGraphPictures);
	}

	private void buildContentionGraphPicture(ActionGraphSection section, String principal, Map<String, ExcelGraphPicture> contentionGraphPictures) throws JzrException {
		if (!this.sheetCfg.isContentionGraphDisplayed())
			return;
		...
		ExcelGraphPicture picture = contentionGraphGenerator.generateGraphPicture(principal, principal, section);
		if (picture != null)
			contentionGraphPictures.put(principal, picture);
	}

	private void buildFunctionGraphPicture(ActionGraphSection section, String principal, Map<String, ExcelGraphPicture> graphPictures) throws JzrException {
		if(!this.sheetCfg.isFunctionGraphDisplayed())
			return;
		...
		ExcelGraphPicture picture = functionGraphGenerator.generateGraphPicture(principal, principal, section);
		if (picture != null)
			graphPictures.put(principal, picture);
	}
	...
	private int displayHeaders(Sheet sheet, int linePos) {
		...
		if(this.sheetCfg.isFunctionGraphDisplayed()){
	    	...
	    	cell = addHeaderCell(row, rowPos++, ""); // graph picture
		}
		
		if(this.sheetCfg.isContentionGraphDisplayed()){
	    	sheet.setColumnWidth(rowPos, 5*256);
	    	cell = addHeaderCell(row, rowPos++, ""); // contention graph picture
		}
        
    	sheet.setColumnWidth(rowPos, 50*256);
    	cell = addHeaderCell(row, rowPos++, "Action / Principal\r\nfunction");
        wrapText(cell);
	...
	private void displayTag(String principal, Tag tag, int tagActionStackCount, int actionStackCount, int globalActionsStackSize, Sheet sheet, int groupCount, int linePos, Map<String, ExcelGraphPicture> graphPictures, Map<String, ExcelGraphPicture> contentionGraphPictures, boolean first) {
    	Row row = sheet.createRow(linePos);
    	int rowPos=0;

		if(this.sheetCfg.isFunctionGraphDisplayed()){
			if (first)
				rowPos = displayGraphPictureLink(graphPictures, principal, row, rowPos, groupCount);
			else
		...
		if(this.sheetCfg.isContentionGraphDisplayed()){
			if (first)
				rowPos = displayGraphPictureLink(contentionGraphPictures, principal, row, rowPos, groupCount);
			else
				addEmptyCell(row, rowPos++, getCellStyleNumberReference(groupCount));
		}
    	
        addCell(row, rowPos++, principal, getCellStyleReference(groupCount));
	...
	private int displayGraphPictureLink(Map<String, ExcelGraphPicture> graphPictures, String principal, Row row, int rowPos, int groupCount) {
		...
		ExcelGraphPicture picture = graphPictures.get(principal);
		if (picture != null)
