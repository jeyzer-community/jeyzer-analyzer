...
public class MemoryPoolHeapRule extends AbstractNumericDisplayHeaderRule implements Header {
	
	public static final String RULE_NAME = "memory_pool_heap";

	public static final String CELL_LABEL_COMMENT = "Heap space usage in Mb\n"
	+ "Formula = Old Gen used space + Eden used space";	
	...
	public MemoryPoolHeapRule(ConfigSheetHeader headerCfg, SheetDisplayContext context) {
	...
	public boolean apply(List<ThreadDump> dumps, List<Cell> cells, HeaderFunction function) {
		Cell cell;
		Long oldGenValue, edenSpaceValue, heapValue;
		Long prevHeapValue = (long) -1;
		Double value;
		boolean available;
		...
		for (int j=0; j< dumps.size(); j++){
			...
			oldGenValue = memPools.getOldGenUsedValue();
			edenSpaceValue = memPools.getYoungSpaceUsedValue();
			available = !(oldGenValue == null || oldGenValue == -1 
					|| edenSpaceValue == null || edenSpaceValue == -1);
			if (available){
				heapValue = oldGenValue + edenSpaceValue;
				// display in Mb
				heapValue = convertToMb(heapValue);
				value = (double) heapValue;
			}
