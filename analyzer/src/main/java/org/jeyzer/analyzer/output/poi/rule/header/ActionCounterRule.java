...
public class ActionCounterRule extends AbstractNumericDisplayHeaderRule implements Header{

	public static final String RULE_NAME = "action_counter";
	private static final String DISPLAY_NAME = "Actions";
	public static final String CELL_LABEL_COMMENT = "Number of detected actions (active threads).";
	...
	public ActionCounterRule(ConfigSheetHeader displayCfg, SheetDisplayContext context) {
	...
	public boolean apply(List<ThreadDump> dumps, List<Cell> cells, HeaderFunction function) {
		...
		for (int j=0; j< dumps.size(); j++){
			...
			value = (double)dumps.get(j).getWorkingThreads().size();
			countValue = value; 
