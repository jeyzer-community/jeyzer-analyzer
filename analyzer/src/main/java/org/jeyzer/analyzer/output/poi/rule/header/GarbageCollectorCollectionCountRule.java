...
public class GarbageCollectorCollectionCountRule extends AbstractGarbageCollectionRule implements Header {

	public static final String RULE_NAME = "garbage_collection_count";

	public static final String CELL_LABEL_COMMENT = "Number of garbage collections since the last JZR recording snapshot";	
	...
	public GarbageCollectorCollectionCountRule(ConfigSheetHeader headerCfg, SheetDisplayContext context) {
	...
	public boolean apply(List<ThreadDump> dumps, List<Cell> cells, HeaderFunction function) {
		...
		for (int j=0; j< dumps.size(); j++){
			...
			countValue = accessCollectionCount(dumps.get(j));
			
			available = !(countValue == -1);
			if (available){
				value = countValue;
			}
	...
	private long accessCollectionCount(ThreadDump threadDump) {
		long countValue = -1;
		...
		if (DISPLAY_GC_ALL.equals(gc)){
			countValue = garbageCollectionData.getGcCount();
		}else{
			GarbageCollectorInfo gcInfo = accessGarbageCollectionInfo(threadDump);
			if (gcInfo != null)
				countValue = gcInfo.getCollectionCount();
		}
