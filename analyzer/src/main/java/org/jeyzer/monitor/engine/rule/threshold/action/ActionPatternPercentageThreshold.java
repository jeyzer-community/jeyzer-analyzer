...
public class ActionPatternPercentageThreshold extends ActionPercentageThreshold {
	
	public ActionPatternPercentageThreshold(ConfigMonitorThreshold thCfg, SubLevel defaultSubLevel) {
		super(thCfg, Scope.ACTION, MatchType.PATTERN_PERCENTAGE, defaultSubLevel);
	}
	
	@Override
	public String getDisplayCondition() {
		ConfigMonitorTaskThreshold taskCfg = (ConfigMonitorTaskThreshold) this.cfg; 
		return "Percentage of \"" + taskCfg.getPattern() + "\" >= " + taskCfg.getPercentageInAction();
	}
	...
	protected boolean matchCondition(MonitorTaskRule rule, ThreadStack stack) {
		...
		return patternProvider.matchPattern(stack, this.cfg.getPattern());
	}
