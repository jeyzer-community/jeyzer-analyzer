...
public class SystemSignalThreshold extends MonitorSystemThreshold{

	public static final String THRESHOLD_NAME = "system signal";
	
	public SystemSignalThreshold(ConfigMonitorThreshold thCfg, SubLevel defaultSubLevel) {
		super(thCfg, Scope.SYSTEM, MatchType.SIGNAL, defaultSubLevel);
	}

	@Override
	public void applyCandidacy(MonitorSystemRule rule, JzrMonitorSession session, Multimap<String, MonitorSystemEvent> events) {
		SignalSystemProvider valueProvider = (SignalSystemProvider) rule;

		MonitorSystemEvent event = null;
		JzrSession jzrSession = (JzrSession) session;
		
		if (valueProvider.matchSignal(jzrSession)){
