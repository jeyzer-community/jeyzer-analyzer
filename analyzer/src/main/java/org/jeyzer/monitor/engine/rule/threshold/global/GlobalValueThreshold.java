...
public class GlobalValueThreshold extends MonitorSessionThreshold{

	public static final String THRESHOLD_NAME = "global value";
	
	public GlobalValueThreshold(ConfigMonitorThreshold thCfg, SubLevel defaultSubLevel) {
		super(thCfg, Scope.GLOBAL, MatchType.VALUE, defaultSubLevel);
	}
	...
	public void applyCandidacy(MonitorSessionRule rule, JzrMonitorSession session, Multimap<String, MonitorSessionEvent> events) {
		...
		for (ThreadDump dump : session.getDumps()){
			if (valueProvider.matchValue(dump, this.cfg.getValue())){
