...
public class SystemPatternThreshold extends MonitorSystemThreshold{

	public static final String THRESHOLD_NAME = "system pattern";	
	
	public SystemPatternThreshold(ConfigMonitorThreshold thCfg, SubLevel defaultSubLevel) {
		super(thCfg, Scope.SYSTEM, MatchType.PATTERN, defaultSubLevel);
	}

	@Override
	public void applyCandidacy(MonitorSystemRule rule, JzrMonitorSession session, Multimap<String, MonitorSystemEvent> events) {
		PatternSystemProvider patternProvider = (PatternSystemProvider) rule;

		MonitorSystemEvent event = null;
		JzrSession jzrSession = (JzrSession) session;
		
		if (patternProvider.matchPattern(jzrSession, this.cfg.getPattern())){
