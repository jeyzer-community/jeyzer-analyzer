...
import org.jeyzer.monitor.impl.event.task.ContentionTypePatternTaskEvent;

public class ContentionTypeTaskRule  extends MonitorTaskRule implements PatternTaskProvider {

	public static final String RULE_NAME = "Contention type pattern";	
	
	public static final String RULE_CONDITION_DESCRIPTION = "Task contention type contains the regex (pattern).";
	public static final String RULE_NARRATIVE = "The " + RULE_NAME + " rule permits to detect a particular contention occurence at thread level. "
	...
	private String contentionDisplayName; // mandatory
	
	public ContentionTypeTaskRule(ConfigMonitorRule def) throws JzrInitializationException {
		super(def, def.getExtraInfo() + " " + RULE_NAME.toLowerCase(), RULE_CONDITION_DESCRIPTION);
		this.contentionDisplayName = def.getExtraInfo();
		if (this.contentionDisplayName == null)
	...
	public boolean matchPattern(ThreadStack stack, Pattern pattern) {
		for (String contentionType : stack.getContentionTypeTags()){
			if (pattern.matcher(contentionType).find())
				return true;
		}
		return false;
	}
	...
	public MonitorTaskEvent createTaskEvent(MonitorEventInfo info, ThreadAction action, ThreadStackHandler stack) {
		return new ContentionTypePatternTaskEvent(
				info,
				action,
				stack,
				this.contentionDisplayName
			);
	}
