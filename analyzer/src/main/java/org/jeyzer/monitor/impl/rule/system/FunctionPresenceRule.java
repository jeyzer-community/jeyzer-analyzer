...
import org.jeyzer.monitor.impl.event.system.FunctionPresenceEvent;

import com.google.common.collect.Multiset;

public class FunctionPresenceRule extends MonitorSystemRule implements SignalSystemProvider{

	public static final String RULE_NAME = "Function presence";
	
	public static final String RULE_NARRATIVE = "The " + RULE_NAME + " rule permits to detect the execution of a function.\n"
			+ " Interesting to detect for example any deprecated code usage.";
	
	private String function;
	private String paramDisplay;
	
	public FunctionPresenceRule(ConfigMonitorRule def) throws JzrInitializationException {
		super(def, RULE_NAME + " : " + ((ConfigParamMonitorRule)def).getDisplayName(), 
				((ConfigParamMonitorRule)def).getDisplayName() + " function is detected.");
	...
	public boolean matchSignal(JzrSession session) {
    	...
    	return functionTags.count(functionTag) > 0;
	}

	@Override
	public MonitorSystemEvent createSystemEvent(MonitorEventInfo info) {
		return new FunctionPresenceEvent(
				this.paramDisplay,
				info,
				this.function
			);
