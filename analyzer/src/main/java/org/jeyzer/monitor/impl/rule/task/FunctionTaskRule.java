...
import org.jeyzer.monitor.impl.event.task.FunctionPatternTaskEvent;

public class FunctionTaskRule  extends MonitorTaskRule implements PatternTaskProvider {

	public static final String RULE_NAME = "Function pattern";
	
	public static final String RULE_CONDITION_DESCRIPTION = "Task function contains the regex (pattern).";
	public static final String RULE_NARRATIVE = "The " + RULE_NAME + " rule permits to detect function apperance at thread level. "
	...
	private String functionDisplayName; // mandatory
	
	public FunctionTaskRule(ConfigMonitorRule def) throws JzrInitializationException {
		super(def, def.getExtraInfo() + " " + RULE_NAME.toLowerCase(), RULE_CONDITION_DESCRIPTION);
		this.functionDisplayName = def.getExtraInfo();
		if (this.functionDisplayName == null)
	...
	public MonitorTaskEvent createTaskEvent(MonitorEventInfo info, ThreadAction action, ThreadStackHandler stack) {
		return new FunctionPatternTaskEvent(
				info,
				action,
				stack,
