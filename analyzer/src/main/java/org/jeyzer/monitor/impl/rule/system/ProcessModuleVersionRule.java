...
import org.jeyzer.monitor.impl.event.system.ProcessModuleVersionEvent;

public class ProcessModuleVersionRule extends MonitorSystemRule implements SignalSystemProvider{

	public static final String RULE_NAME = "Process module version";
	
	public static final String RULE_NARRATIVE = "The " + RULE_NAME + " rule permits to check the presence of a Java module version "
			+ "and provide a message about it like an End Of Life warning, or a security issue requiring an immediate module upgrade."
			+ "This rule must get associated at least to one process module version sticker. It has no check body and relies only on the sticker(s). "
	...
	public ProcessModuleVersionRule(ConfigMonitorRule def) throws JzrInitializationException {
	...
	public MonitorSystemEvent createSystemEvent(MonitorEventInfo info) {
		return new ProcessModuleVersionEvent(this.extraInfo, info);
	}
