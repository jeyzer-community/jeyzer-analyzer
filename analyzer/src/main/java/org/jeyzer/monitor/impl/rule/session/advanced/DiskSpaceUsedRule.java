...
import org.jeyzer.monitor.impl.event.session.advanced.DiskSpaceUsedEvent;

public class DiskSpaceUsedRule extends MonitorSessionRule implements ValueSessionProvider{

	public static final String RULE_NAME = "Disk space used";
	
	public static final String RULE_NARRATIVE = "The " + RULE_NAME + " rule permits to detect high usage of disk space under a particular partition or directory, based on the size of used disk space there.";
	
	private String diskSpaceName;
	private String displayName;
	
	public DiskSpaceUsedRule(ConfigMonitorRule def)
			throws JzrInitializationException {
		super(def, 
				RULE_NAME + " : " + ((ConfigParamMonitorRule)def).getDisplayName(), 
				((ConfigParamMonitorRule)def).getDisplayName() + " " + RULE_NAME.toLowerCase() + " is greater or equal to (value) Gb.");
	...
	public boolean matchValue(ThreadDump dump, long value) {
		...
		if (info == null)
			return false;
		return FormulaHelper.convertToGb(info.getUsedSpace()) >= value;
	}
